...
The server and client use the **same** JAR file. Configuration is server-side controlled. The client must also install createsifter, Create, and Fabric API, and all versions **must** match the server's.

服务器与客户端使用**同一份**jar文件，配置由服务器主导，客户端也要装 createsifter + 机械动力 + Fabric API，版本**必须**与服务器一致。

---

## Compatibility / 兼容性（中文在这段的下面哦）

English:
> Compiled for **Minecraft 1.20.1 · Create 6.0.8.1+ · Fabric**.

### I. Confirmed Compatible✅ (Tested)
...
### II. Theoretically Compatible (Not individually tested; derived from official compatibility boundaries)
...
### III. Unknown Compatibility⚠️ (Untested; potential points of contact)
...
- **Create addons with deep stress-network / block-entity integration:** Interactive's detailed interaction, TFMG, etc.
- Any other niche mods not listed

### IV. Incompatible❌ (Theoretical / Tested)
...
**Tip**: If your mechanical power version is 6.0.7–6.0.8, you need to upgrade to 6.0.8.1 to use this mod.

> 1. From the mods/ folder, delete the old create-fabric-*.jar and place the new (6.0.8.1) .jar file.
> 2. Delete any standalone flywheel, ponder, porting_lib_*, and registrate-fabric .jar files in the `mods` folder, if present.
> 3. Leftover standalone libraries from the Create 6 era are the most common source of errors (old Flywheel/Ponder conflict with new Create). Check whether other Create add-on mods meet the minimum version requirements for the new Create (see the compatibility table, Category 4, e.g., Railways ≥ 1.5.3, Copycats > 1.1.1, etc.) — upgrade them simultaneously if they don't.
> 4. Launch verification: enter the game → enter a world → check the log for no red errors and ensure add-on mods function properly.
> 5. Enjoy your wonderful gaming time!

---
中文：
> 以下兼容数据基于 **Minecraft 1.20.1 · Create 6.0.8.1+ · Fabric** 环境整理。
...
> 1、从 mods/ 删除旧版 create-fabric-*.jar，并且放入新版（6.0.8.1）jar
> 2、删除 mods 里独立的 flywheel、ponder、porting_lib_*、registrate-fabric jar（如有）
> 3、残留旧版独立库是 Create 6 时代最常见的报错源头（旧 Flywheel/Ponder 与新 Create 冲突），检查其他 Create 附加模组版本是否满足新 Create 的下限（详见兼容性总表第四类，如 railways≥1.5.3、copycats>1.1.1 等）——不满足的同步升级
> 4、启动验证：进游戏 → 进世界 → 看日志无红字、附加模组功能正常。
> 5、开启美好游戏时光~

---

## License & credit / 许可与致谢

**Note / 注意：**

**The direct upstream **oierbravo** uses **LGPL-3.0** (Forge version); the Fabric source repo this adaptation is based on (**Shulej**) is **MIT**. The difference between the two upstream licenses is acknowledged; all upstream notices have been retained.**

**最上游 **oierbravo** 采用 **LGPL-3.0**（Forge 版）；本适配版基于的 Fabric 源码仓库（**Shulej**）为 **MIT**。两方协议差异已知悉，本适配版已保留全部上游声明。**
